...
app.post("/signup", async function(req, res){
    ...
    //const parsedData = requiredBody.parse(req.body);       //the problem with this line of code is it will either return you the data , if everything is correct or it will thrown an error.so you need to put it in try catch block. so it is preferable that to use the below line of code 
    const parsedDataWithSuccess = requiredBody.safeParse(req.body);

    if(!parsedDataWithSuccess.success){
        res.json({
            message: "Incorrect format",
            error: parsedDataWithSuccess.error            //with this line the user can get to know that what is the error he is doing, instead of just incorrect format 
        })
        return 
    }
    ...
    const password = req.body.password;       //string, 10 chars, 1 spl, 1 uppercase, 1 lowercase 
